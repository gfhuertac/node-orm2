...
function Instance(Model, opts) {
	...
	var saveNew = function (saveOptions, data, cb) {
		...
		var wrap = function (save_err, info) {
			if (save_err) {
				return saveError(cb, save_err);
			}

			opts.changes.length = 0;
                        opts.lastUpdate = new Date().getTime();
			for (i = 0; i < opts.keyProperties.length; i++) {
				prop = opts.keyProperties[i];
		...
		if (opts.transaction) {
			opts.driver.insert(opts.table, data, opts.keyProperties, opts.transaction, wrap);
		} else {
			opts.driver.insert(opts.table, data, opts.keyProperties, wrap);
		}
	};
	var savePersisted = function (saveOptions, data, cb) {
		...
		if (opts.changes.length === 0) {
			...
			if (opts.transaction) {
				opts.driver.update(opts.table, changes, conditions, opts.transaction, wrap);
			} else {
				opts.driver.update(opts.table, changes, conditions, wrap);
			}
		}
	};
	...
	var saveInstanceExtra = function (cb) {
		...
		if (opts.transaction) {
			opts.driver.update(opts.extra_info.table, data, conditions, opts.transaction, wrap);
		} else {
			opts.driver.update(opts.extra_info.table, data, conditions, wrap);
		}
	};
	var removeInstance = function (cb) {
		...
		Hook.wait(instance, opts.hooks.beforeRemove, function (err) {
			...
			if (opts.transaction) {
				opts.driver.remove(opts.table, conditions, opts.transaction, wrap);
			} else {
				opts.driver.remove(opts.table, conditions, wrap);
			}
		});
	};
	...
	Object.defineProperty(instance, "lastUpdate", {
		value: function () {
			return opts.lastUpdate;
		},
		enumerable: false
	});
