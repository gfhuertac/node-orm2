...
function extendInstance(Model, Instance, Driver, association, opts, createInstance) {
	...
	Object.defineProperty(Instance, association.hasAccessor, {
		value: function () {
			...
			var wrap = function (err, instances) {
				if (err) {
					return cb(err);
				}
				if (!Instances.length) {
			...
			if (Instance.transaction) {
				association.model.find(conditions, options, Instance.transaction, wrap);
			} else {
				association.model.find(conditions, options, wrap);
			}
			return this;
		},
		enumerable: false,
				...
				var saveNextAssociation = function () {
					...
					var saveAssociation = function (err) {
						...
						Association.save(function (err) {
							...
							if (Instance.transaction) {
								Driver.insert(association.mergeTable, data, null, Instance.transaction, wrap);
							} else {
								Driver.insert(association.mergeTable, data, null, wrap);
							}
						});
					};
