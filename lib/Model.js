...
function Model(opts) {
	...
	model.get = function () {
		...
		if (ids[ids.length - 1] === undefined) {
			ids.pop();
		} else if (typeof ids[ids.length - 1] === "object" && Utilities.isTransaction(ids[ids.length - 1])) {
			transaction = ids.pop();
		}
		...
		if (transaction) {
			opts.driver.find(model_fields, opts.table, conditions, { limit: 1 }, transaction, wrap); 
		} else {
			opts.driver.find(model_fields, opts.table, conditions, { limit: 1 }, wrap); 
		}
	...
	model.count = function () {
		var conditions = null;
		var cb         = null;
		var transaction = Utilities.getTransaction(arguments);
		...
		if (transaction) {
			opts.driver.count(opts.table, conditions, {}, transaction, wrap);
		} else {
			opts.driver.count(opts.table, conditions, {}, wrap);
		}
		return this;
	};
	...
	model.clear = function (cb) {
		...
		if (transaction) {
			opts.driver.clear(opts.table, transaction, wrap);
		} else {
			opts.driver.clear(opts.table, wrap);
		}
