...
var switchableFunctions = {
	pool: {
		...
		execSimpleQuery: function (query, transaction, cb) {
			if (this.opts.debug) {
				require("../../Debug").sql('postgres', query);
			}
			if (cb === undefined && typeof transaction === 'function') {
				cb = transaction;
...
Driver.prototype.find = function (fields, table, conditions, opts, transaction, cb) {
	if (cb === undefined && typeof transaction === 'function') {
		cb = transaction;
		transaction = undefined;
	}
...
Driver.prototype.count = function (table, conditions, opts, transaction, cb) {
	if (cb === undefined && typeof transaction === 'function') {
		cb = transaction;
		transaction = undefined;
	}
...
Driver.prototype.insert = function (table, data, keyProperties, transaction, cb) {
	if (cb === undefined && typeof transaction === 'function') {
		cb = transaction;
		transaction = undefined;
	}
...
Driver.prototype.update = function (table, changes, conditions, transaction, cb) {
	if (cb === undefined && typeof transaction === 'function') {
		cb = transaction;
		transaction = undefined;
	}
...
Driver.prototype.remove = function (table, conditions, transaction, cb) {
	if (cb === undefined && typeof transaction === 'function') {
		cb = transaction;
		transaction = undefined;
	}
...
Driver.prototype.clear = function (table, transaction, cb) {
	if (cb === undefined && typeof transaction === 'function') {
		cb = transaction;
		transaction = undefined;
	}
